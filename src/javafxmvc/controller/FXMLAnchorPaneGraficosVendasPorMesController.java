...
public class FXMLAnchorPaneGraficosVendasPorMesController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // Obtém an array com nomes dos meses
        ...
        //Passa por todos o dados do meu Map
        for (Map.Entry<Integer, ArrayList> dadosItem : dados.entrySet()) {
            XYChart.Series<String, Integer> series = new XYChart.Series<>();//cria uma nova serie de dados para cada elemento do map 
            ...
            for (int i = 0; i < dadosItem.getValue().size(); i = i + 2) {
                ...
                mes = retornaNomeMes((int) dadosItem.getValue().get(i));//chama o switch para os meses 
                quantidade = (Integer) dadosItem.getValue().get(i + 1);

                series.getData().add(new XYChart.Data<>(mes, quantidade));
            }
            barChart.getData().add(series); //adiciona a serie de dados ao ano 
        }
